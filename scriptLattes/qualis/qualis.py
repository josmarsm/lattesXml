    ...
    def carregarQualis(self, arquivo):
        ...
        if (not arquivo == ''):
            ...
            for i in xrange(plan.nrows):
				...
				rows = plan.row_values(i)

				#print rows
				sigla = rows[0].rstrip()  # ISSN de periodicos ou SIGLA de congressos
				nome = rows[1].rstrip()  # Nome do periodico ou evento
				qualis = rows[2].rstrip()  # Estrato Qualis
